...
async def call_agent(query, session_id):
    ...
    if not correct:
        ...
        content = types.Content(role='user', parts=[types.Part(text=random.choice(complaints))])
        events = runner.run(user_id=USER_ID, session_id=session_id, new_message=content)
        # Consume events
        for event in events:
            pass
...
async def main(max_traces, max_traces_per_session):
    ...
    while total_traces_complete < max_traces:
        ...
        session = await session_service.create_session(user_id=USER_ID, app_name=root_agent.name)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run the calculator agent and generate trace data.")
    parser.add_argument("--max-traces", type=int, default=5, help="Maximum number of traces to generate (default: 5)")
    parser.add_argument("--max-traces-per-session", type=int, default=2, help="Maximum traces per session (default: 2)")
